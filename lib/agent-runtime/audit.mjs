...
function writeAuditLine(full, entry) {
...
export function appendAudit(kbRoot, entry) {
  try {
    ...
    // read-last-hash + append must be one critical section. Callers hold at
    // most a *per-agent* lock, so two agents (or the CLI and the MCP server)
    // appending at the same time both read the same prev_hash, both write it,
    // and verifyAuditChain() reports prev_hash mismatch from that point on —
    // permanently, since the chain is never rewritten. Reproduced with six
    // concurrent writers: the chain broke 70 entries in.
    let lock = null
    try {
      lock = acquireLock(kbRoot, 'audit-log', { maxAgeMs: 10_000, retries: 40, retryDelayMs: 25 })
    } catch {
      // Lock unobtainable (stuck holder). Losing the entry is worse than a
      // chain break — an unsigned gap is visible, a missing record is not.
      console.error('[audit] appendAudit: proceeding without lock (contended)')
    }
    try {
      writeAuditLine(full, entry)
    } finally {
      lock?.release()
    }
  } catch (err) {
