...
// Newest-first over `created_at`, deterministically.
//
// The previous comparator was `String(b).localeCompare(String(a))`, which had
// two defects. An item with no `created_at` stringifies to "undefined", which
// sorts *above* every ISO date, so malformed items were returned as the most
// recent and pushed real items off the end of `slice(0, limit)`. And
// localeCompare() honours the runtime's default locale, so the same channel
// listed in a different order on a different machine — the exact call
// lib/repo-runtime/queries.mjs:byString refuses to use, for the same reason.
//
// Unparseable or absent dates sort last rather than first; `id` breaks ties so
// the order is total and stable regardless of readdir order.
export function compareBusItemsByCreatedAt(a, b) {
  const ta = Date.parse(a?.meta?.created_at)
  const tb = Date.parse(b?.meta?.created_at)
  const aBad = !Number.isFinite(ta)
  const bBad = !Number.isFinite(tb)
  if (aBad !== bBad) return aBad ? 1 : -1
  if (!aBad && ta !== tb) return tb - ta
  const ia = String(a?.meta?.id ?? '')
  const ib = String(b?.meta?.id ?? '')
