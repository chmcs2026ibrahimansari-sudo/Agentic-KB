...
// Classes classFor() only returns on an explicit path match — see bucketRank.
const PATH_DERIVED_CLASSES = new Set(['profile', 'hot', 'working', 'rewrite', 'bus'])
...
function isStale(kbRoot, relPath, freshnessThreshold) {
  ...
  try {
    ...
    if (updated) {
      // An unparseable date yields NaN, and `NaN < cutoff` is false — which
      // would mark the file permanently fresh and skip the mtime fallback.
      // Only trust the frontmatter date when it actually parses.
...
function bucketRank(item, priorityOrder) {
  ...
  // Path include rules (rule.path / rule.path glob) produce candidates with
  // no `class` and a reason of `policy path …`, so a profile.md or hot.md
  // pulled in by an explicit path rule matched none of the branches above
  // and fell through to rank 999 — behind bus subscriptions, and therefore
  // first to be dropped under budget pressure. That is the inverse of
  // CANONICAL_ORDER, which puts profile and hot ahead of everything but
  // task-local.
  //
  // Only trust classFor where it actually matched a path pattern: 'learned'
  // is also its no-match default, so honouring that would silently re-rank
  // every unclassified include.
  const inferred = classFor(item.path)
  if (PATH_DERIVED_CLASSES.has(inferred)) {
    const j = order.indexOf(nameMap[inferred] || inferred)
    if (j >= 0) return j
  }
...
// ─── Reasoning-mode conditional includes ────────────────────────────────────
// Each mode adds opt-in supporting context that only matters in that mode.
// Files are added to candidates with priority 75 (mid-low) so contract-declared
// includes still rank higher.

function existingPaths(kbRoot, dir, max = 5) {
  const full = path.join(kbRoot, dir)
...
function resolveModeIncludes(kbRoot, mode) {
  const out = []
  const add = (relPath, why) => out.push({
    path: relPath, reason: `mode:${mode} → ${why}`, priority: 75, bucket: 'mode', rule: null,
  })
  switch (mode) {
    case 'critique': {
      const lint = path.join(kbRoot, 'wiki/lint-report.md')
      if (fs.existsSync(lint)) add('wiki/lint-report.md', 'lint findings')
      for (const p of existingPaths(kbRoot, 'wiki/syntheses', 3)) {
        if (/lint-/.test(p)) add(p, 'lint synthesis')
      }
      break
    }
    case 'plan': {
      for (const p of existingPaths(kbRoot, 'wiki/decisions', 5)) add(p, 'related ADRs')
      break
    }
    case 'compare': {
      for (const p of existingPaths(kbRoot, 'wiki/evaluations', 5)) add(p, 'evaluation source')
      break
    }
    case 'synthesize':
    case 'answer':
    case 'delegate':
    default:
      // No-op — these modes use the default include set.
      break
  }
  return out
}
...
export function loadAgentContext(kbRoot, contract, vars = {}) {
  ...
  // ── Reasoning-mode conditional includes ──────────────────────────────────
  // Mode passed via vars.mode. Only applies when contract.reasoning_policy
  // declares the mode in its modes[] list.
  const policyModes = contract.reasoning_policy?.modes || []
  const requestedMode = vars.mode || contract.reasoning_policy?.default_mode
  if (requestedMode && policyModes.includes(requestedMode)) {
    for (const r of resolveModeIncludes(kbRoot, requestedMode)) push(r)
  }
