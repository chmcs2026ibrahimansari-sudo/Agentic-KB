...
// Real YAML parser — handles block scalars, anchors, aliases, comments.
// Replaces 130 lines of hand-rolled parseYamlLite.
function parseYamlReal(text) {
  let parsed
  try {
    parsed = yamlParse(text)
  } catch (e) {
    // A syntactically broken contract must fail with a clear message, not a
    // raw YAMLParseError leaking out of loadContract into web/MCP callers.
    throw new Error(`invalid YAML in contract: ${e.message}`)
  }
  if (parsed == null) return {}
  // A contract whose document is a scalar or array is not a mapping. Returning
  // it would make `contract.agent_id = ...` in loadContract throw an opaque
  // "Cannot create property on string" TypeError; reject it explicitly.
  if (typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new Error('contract must be a YAML mapping')
  }
  return parsed
}

// Legacy hand-rolled parser kept exported for the test that pins its behavior.
// Do NOT use in production paths — call parseYamlReal instead.
...
// Agent ids arrive from URL params (web /api/agents/[id]/*) and MCP tool
// arguments, so treat them as untrusted: a separator or dot-segment would
// make the path below resolve outside config/agents/ and let a caller load
// any .yaml on disk as a "contract". Same shape /api/agents/[id]/diff enforces.
...
export function loadContract(kbRoot, agentId) {
  ...
  // listContracts accepts both .yaml and .yml files, so try both here too —
  // otherwise a .yml contract is listed but silently unloadable (listContracts
  // filters out the resulting null and the agent just disappears).
  let file = path.join(kbRoot, 'config', 'agents', `${agentId}.yaml`)
  if (!fs.existsSync(file)) file = path.join(kbRoot, 'config', 'agents', `${agentId}.yml`)
  if (!fs.existsSync(file)) return null
  const text = fs.readFileSync(file, 'utf8')
  const contract = parseYamlReal(text)
...
export function validateContract(c) {
  ...
  if (c.vault_writes != null && !Array.isArray(c.vault_writes)) errs.push('vault_writes must be array')
  if (c.vault_id != null && typeof c.vault_id !== 'string') errs.push('vault_id must be string when set')
  if (c.permitted_cross_tier_reads != null && !Array.isArray(c.permitted_cross_tier_reads)) {
    errs.push('permitted_cross_tier_reads must be array of glob patterns')
  }
  ...
  if (Array.isArray(c.vault_writes)) {
    for (const p of c.vault_writes) {
      if (typeof p !== 'string') { errs.push(`vault_writes entry not string: ${p}`); continue }
      if (p.startsWith('/') || p.includes('..')) errs.push(`vault_writes unsafe pattern: ${p}`)
    }
  }
