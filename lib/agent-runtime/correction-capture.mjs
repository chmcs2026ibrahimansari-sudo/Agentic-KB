...
// Frontmatter below is hand-built line by line; a newline in a caller-supplied
// value would terminate its line early and inject arbitrary frontmatter keys
// (same class of bug fixed in vault-writeback and the ADR emitter).
function oneLine(s) {
...
export function captureCorrection(kbRoot, contract, {
  ...
  if (!DURABILITY_CLASSES.includes(effectiveDurability)) {
    throw new Error(`Invalid durability: '${effectiveDurability}'. Must be one of: ${DURABILITY_CLASSES.join(', ')}`)
  }
  ...
  const renderContent = (id) => [
    ...
    `correction_id: ${id}`,
    `agent_id: ${agentId}`,
    taskId ? `task_id: ${oneLine(taskId)}` : null,
    `type: ${type}`,
    `durability: ${effectiveDurability}`,
    `confidence: ${typeof confidence === 'string' ? oneLine(confidence) : numConf.toFixed(2)}`,
    `confidence_numeric: ${numConf.toFixed(3)}`,
    `promote_candidate: ${promoteCandidate}`,
    `captured_at: ${now}`,
    sources.length > 0 ? `sources:\n${sources.map(s => `  - ${oneLine(s)}`).join('\n')}` : null,
    '---',
    '',
    `# Correction: ${type}`,
    '',
  ...
  // within one process, so two processes (MCP server, CLI, web) capturing in
  // the same second would silently overwrite each other with a plain write.
  let finalId = correctionId
  let finalPath = correctionPath
  for (let n = 2; ; n++) {
    try {
      fs.writeFileSync(finalPath, renderContent(finalId), { encoding: 'utf8', flag: 'wx' })
      break
    } catch (err) {
      if (err.code !== 'EEXIST' || n > 20) throw err
      finalId = `${correctionId}-${n}`
      finalPath = path.join(correctionsDir, `${finalId}.md`)
    }
  }

  return { correctionId: finalId, path: path.relative(kbRoot, finalPath) }
}
...
export function listCorrections(kbRoot, contract, {
  ...
  for (const file of files) {
    ...
    try {
      // Read the full file: a long sources: list pushes frontmatter past any
      // fixed byte cap, and a truncated header parses as {} (metadata lost).
...
export function getCorrection(kbRoot, contract, correctionId) {
  const { agentId, tier } = { agentId: contract.agent_id, tier: contract.tier }
  // correctionId is joined into a filesystem path — reject anything that is
  // not a generated correction id so '../..' cannot escape the corrections dir.
  if (!/^correction-[A-Za-z0-9-]+$/.test(String(correctionId))) return null
