...
function pricingFor(model) {
  for (const k of Object.keys(PRICING)) {
    if (k !== 'default' && model && model.includes(k)) return PRICING[k]
  }
  // Family fallback: new model ids (e.g. claude-opus-4-8) must not silently
  // fall through to sonnet-level default pricing — for opus that undercounts
  // spend 5x against the daily cap.
  if (model && model.includes('opus')) return PRICING['claude-opus-4-6']
  if (model && model.includes('haiku')) return PRICING['claude-haiku-4-5']
...
export function spendForDay(kbRoot, isoDate = new Date().toISOString().slice(0, 10)) {
  return readCostLog(kbRoot)
    .filter(e => typeof e.ts === 'string' && e.ts.startsWith(isoDate))
    .reduce((s, e) => s + (e.cost_usd || 0), 0)
}

export function spendForMonth(kbRoot, ymPrefix = new Date().toISOString().slice(0, 7)) {
  return readCostLog(kbRoot)
    .filter(e => typeof e.ts === 'string' && e.ts.startsWith(ymPrefix))
    .reduce((s, e) => s + (e.cost_usd || 0), 0)
}
...
export function summary(kbRoot) {
  // Single pass over one log read — summary() previously re-read the whole
  // JSONL file three times (spendForDay, spendForMonth, byModel).
  const cap = dailyCapUsd()
  const log = readCostLog(kbRoot)
  const dayPrefix = new Date().toISOString().slice(0, 10)
  const monthPrefix = dayPrefix.slice(0, 7)
  let today = 0
  let month = 0
  const byModel = {}
  for (const e of log) {
    ...
    if (typeof e.ts === 'string') {
      if (e.ts.startsWith(dayPrefix)) today += cost
      if (e.ts.startsWith(monthPrefix)) month += cost
    }
    byModel[e.model || 'unknown'] = (byModel[e.model || 'unknown'] || 0) + cost
  }
  return {
