...
export function parseFrontmatter(content) {
  // Normalize the byte-level prologue before the sentinel check. Either of
  // these made an otherwise valid block invisible: the page then parsed as
  // having no frontmatter (memory_class, status, title lost) and every writer
  // prepended a SECOND block on top of the original.
  //   - CRLF: Windows-authored or GitHub-synced files (fixed in 9a2a4fb).
  //   - UTF-8 BOM: same sources, same failure. Reproduced through
  //     repo-runtime/sync.mjs writeImportedDoc() with a BOM-prefixed README
  //     -- the imported doc came out with two stacked frontmatter blocks and
  //     the remote's own keys stranded in the body.
  ...
  while (i < lines.length) {
    ...
    // Hyphens are legal in YAML keys and the wiki uses them
    // (review-cadence-days, source-trust). They were not in the key class, so
    // the line failed to match and was skipped -- the key was invisible to
    // wiki-lint/source-trust, and because updateFrontmatter re-serialises only
    // what it parsed, the next agent write deleted it from the file.
