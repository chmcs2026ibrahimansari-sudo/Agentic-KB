...
// Does a candidate-declared relative source path actually exist in the KB?
// Cited paths are untrusted strings, so keep them inside kbRoot.
function existsUnder(kbRoot, relPath) {
  if (typeof relPath !== 'string' || relPath.length === 0) return false
  if (relPath.includes('\0') || relPath.split('/').includes('..')) return false
  try {
    return fs.existsSync(path.join(kbRoot, relPath))
  } catch {
    return false
  }
}
...
export function scorePromotion(kbRoot, candidate, {
  ...
  if (sources.length > 0) {
    const declared = sources.filter(s => typeof s === 'string' && s.startsWith('wiki/'))
    // Only score sources that exist. getAgeInDays returns 0 ("can't determine
    // age, treat as fresh") when statSync throws, so a candidate citing pages
    // that were never written scored a perfect 1.00 freshness — 0.20 of the
    // weighted total — on phantom evidence.
    const present = declared.filter(s => existsUnder(kbRoot, s))
    const missing = declared.length - present.length
    if (missing > 0) reasons.push(`${missing} cited source path(s) do not exist`)
    if (present.length > 0) {
      const freshScores = present.map(s => scoreFreshness(kbRoot, s).score)
      freshnessScore = freshScores.reduce((a, b) => a + b, 0) / freshScores.length
    } else if (declared.length > 0) {
      // Every cited wiki source is missing: that is worse than citing none.
      freshnessScore = 0
    }
  }
...
function checkCanonicalGates(kbRoot, candidate, { targetPath, contract, freshnessRequired }) {
  ...
  if (freshnessRequired && targetPath) {
    const sources = candidate.sources || candidate.related_sources || []
    const declared = sources.filter(s => typeof s === 'string' && s.startsWith('wiki/'))
    // A source that is not on disk cannot be revalidated, and
    // isFreshForCanonical returns true for it (getAgeInDays treats an
    // unstattable path as age 0) — so the gate used to pass on evidence that
    // was never written. Missing is a failure, not a pass.
    const missing = declared.filter(s => !existsUnder(kbRoot, s))
    if (missing.length > 0) {
      failures.push(`supporting source(s) not found: ${missing.slice(0, 3).join(', ')}`)
    }
    const stale = declared
      .filter(s => existsUnder(kbRoot, s))
...
export function checkContradictions(kbRoot, candidate, { targetPath } = {}) {
  ...
  // Text of the pages we flagged, so the strong-conflict test can compare the
  // candidate against what it supposedly contradicts.
  ...
  if (fs.existsSync(hotPath)) {
    ...
    if (_hasConflict(title, tags, body, hotContent)) {
      ...
      conflictingContent.push(hotContent)
    }
  }
    ...
    if (fs.existsSync(targetDir)) {
      ...
      for (const sibling of siblings) {
        ...
        try {
          ...
          const sibText = content.slice(0, 800).toLowerCase()
          if (overlap.length >= 2 && _hasConflict(title, tags, body, sibText)) {
            const rel = path.relative(kbRoot, sibPath)
            conflictingPages.push(rel)
            conflictingContent.push(sibText)
          }
    ...
    if (fs.existsSync(absPath)) {
      try {
        ...
        if (_hasConflict(title, tags, body, content)) {
          ...
          conflictingContent.push(content)
        }
      } catch (_) { /* skip */ }
    }
  }
  ...
  if (unique.length >= 1 && _hasStrongConflict(body, conflictingContent.join('\n'))) status = 'confirmed'
...
const NEGATION_PATTERNS = [
  // The typographic apostrophe was missing and /\bdon't\b/ was listed twice.
  /\bnot\b/, /\bnever\b/, /\bno\b/, /\bdon['’]t\b/,
  /\bavoid\b/, /\bagainst\b/, /\bwrong\b/, /\bincorrect\b/, /\bfalse\b/,
]
// Both halves must be disjoint strings: ['use', "don't use"] used to be here,
// and since "don't use" contains "use" the pair was satisfied by either half
// on its own — every document mentioning "don't use" matched it.
...
// 'confirmed' means the candidate contradicts the page it was flagged
// against — so the opposing terms have to land on *opposite* sides. This used
// to inspect the candidate's own body twice, so ordinary prose containing a
// negation plus both halves of a pair ("never do X; always do Y") escalated
// itself to confirmed against a page it merely shared title words with, and
// confirmed hard-blocks canonical promotion.
function _hasStrongConflict(body, existingContent = '') {
  const cand = String(body).toLowerCase()
  const other = String(existingContent).toLowerCase()
  if (!NEGATION_PATTERNS.some(re => re.test(cand))) return false
  return OPPOSING_PAIRS.some(([a, b]) =>
    (cand.includes(a) && other.includes(b)) ||
    (cand.includes(b) && other.includes(a)))
}
