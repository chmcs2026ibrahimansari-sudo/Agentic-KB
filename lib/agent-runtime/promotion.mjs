...
// Client-supplied target/rewrite/canonical paths are joined onto kbRoot and
// written with fs — reject anything that could land outside the vault.
//
// Staying inside kbRoot is not enough. Promotion writes a caller-supplied body
// to a caller-supplied path, so "anywhere under kbRoot" included
// config/agents/<id>.yaml (an agent's allowed_writes / forbidden_paths),
// namespaces.json (the web RBAC table), CLAUDE.md and logs/audit.log —
// i.e. promotion could rewrite the very policy that bounds the agent runtime.
// Promotion targets are wiki pages by definition: the default target is
// wiki/system/bus/standards/, rewrites live under wiki/**/rewrites/, and
// canonical docs under wiki/. Constrain to wiki/ here, at the single chokepoint
// shared by the CLI, the MCP server, and the web route.
function assertSafeRelPath(value, kind) {
  if (value == null) return
  if (typeof value !== 'string' || value.length === 0) throw new Error(`invalid ${kind}: empty or non-string`)
  if (path.isAbsolute(value) || value.startsWith('~')) throw new Error(`invalid ${kind}: must be relative to the vault root`)
  if (value.includes('..') || value.includes('\0') || value.includes('\\')) throw new Error(`invalid ${kind}: unsafe characters`)
  if (!value.startsWith('wiki/')) throw new Error(`invalid ${kind}: must be under wiki/`)
  if (!/\.mdx?$/.test(value)) throw new Error(`invalid ${kind}: must be a .md or .mdx page`)
}
...
// Atomic tmp + rename: promoted artifacts, canonical docs, and the sealed
// source/rewrite files are rewritten in place below — a crash mid-write must
// never leave a truncated canonical or a bus item stripped of its metadata.
function atomicWrite(full, data) {
  const tmp = full + '.tmp-' + process.pid + '-' + Date.now()
  fs.writeFileSync(/* turbopackIgnore: true */ tmp, data)
...
function validateApprover(kbRoot, approver, contract, { requiredTier = 'lead' } = {}) {
  ...
  // Try to load the approver's contract; if there is no contract file at all,
  // the approver is a human and bypasses tier checks.
  //
  // The catch used to swallow *every* failure, so it could not tell "no such
  // contract" from "contract exists but failed to parse or validate".
  // loadContract throws on both a malformed YAML body and a failed
  // validateContract, so a real worker-tier agent whose contract had a typo
  // was silently reclassified as a human and skipped min_approver_tier
  // entirely. A broken contract must fail loudly, not open the gate.
  let approverContract
  try {
    approverContract = loadContract(kbRoot, approver)
  } catch (err) {
    throw new Error(
      `Approver ${approver} has a contract that could not be loaded: ${err.message}. ` +
      'Fix the contract or approve as a human identity.'
    )
  }
  if (!approverContract) {
    // No contract file — assume human approver; humans bypass tier checks
    return { kind: 'human', id: approver }
  }
  const approverTier = approverContract.tier
...
function findDuplicateTitle(kbRoot, title, channel, excludeId = null) {
  ...
  const channelsToScan = channel === 'standards' ? ['standards'] : ['standards', channel]
  const dirsToScan = channelsToScan.map(ch => ({
    channel: ch,
    dir: path.join(/* turbopackIgnore: true */ kbRoot, 'wiki', 'system', 'bus', ch),
  }))

  for (const { channel: scanChannel, dir } of dirsToScan) {
    ...
    for (const f of fs.readdirSync(/* turbopackIgnore: true */ dir)) {
      ...
      try {
        ...
        if (candidateTitle && candidateTitle === normalised) {
          // Report the directory the match was actually found in — previously
          // a match in standards/ was misreported under the source channel.
          return path.join('wiki/system/bus', scanChannel, f)
        }
      } catch (err) {
...
export function promoteDiscovery(kbRoot, opts = {}) {
  ...
  assertSafeRelPath(targetPath, 'targetPath')
  assertSafeRelPath(supersedes, 'supersedes')
  ...
  if (!skipScorer && contract) {
    ...
    // 3. Route to review channel unless the scorer cleared the candidate.
    // This was an explicit REVIEW/REJECT deny-list, which let the fifth
    // decision — WORKING_ONLY, the *weakest* non-reject verdict — fall
    // straight through to the wiki write below. A candidate scoring 0.30
    // was promoted while a better one scoring 0.50 was blocked, and a
    // confirmed contradiction with a low score promoted itself with
    // contradiction_status: confirmed in its own frontmatter. Only the two
    // decisions that mean "this is durable knowledge" may proceed.
    const cleared = scoreResult.decision === DECISIONS.LEARNED ||
      scoreResult.decision === DECISIONS.CANONICAL
    if (!cleared) {
  ...
  atomicWrite(targetFull, promoted)
...
export function mergeRewrite(kbRoot, {
  ...
  assertSafeRelPath(rewritePath, 'rewritePath')
  assertSafeRelPath(canonicalPath, 'canonicalPath')
  ...
  // Snapshot previous canonical to archive — exclusive create with bounded
  // suffixes: timestamp() is per-process, so two merges of the same canonical
  // in the same second computed the same archive path and the second plain
  // write replaced the first snapshot (the only copy of that prior canonical
  // once the overwrite below lands). Same class as the retention-snapshot fix.
  if (canonicalBefore) {
    const archiveBase = `wiki/archive/merges/${canonicalPath.replace(/\//g, '__')}-${timestamp()}.md`
    let archiveRel = archiveBase
    for (let n = 2; ; n++) {
      const archiveFull = path.join(/* turbopackIgnore: true */ kbRoot, archiveRel)
      fs.mkdirSync(/* turbopackIgnore: true */ path.dirname(archiveFull), { recursive: true })
      try {
        fs.writeFileSync(/* turbopackIgnore: true */ archiveFull, canonicalBefore, { flag: 'wx' })
        break
      } catch (err) {
        if (err.code !== 'EEXIST' || n > 20) throw err
        archiveRel = archiveBase.replace(/\.md$/, `-${n}.md`)
      }
    }
  ...
  atomicWrite(canFull, newContent)
  const afterHash = hash(newContent)

  // Seal rewrite as merged with back-reference
  atomicWrite(rwFull, updateFrontmatter(rwContent, {
    status: 'merged',
