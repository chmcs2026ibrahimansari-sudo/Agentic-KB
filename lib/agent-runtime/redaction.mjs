...
const DEFAULT_RULES = [
  ...
  // \d(?:[ -]?\d){12,18} rather than (?:\d[ -]?){13,19}: the old form let the
  // final repetition consume its optional separator, so the space *after* the
  // number was swallowed and the redaction glued itself to the next word
  // ("Timestamp 1755331200000 in the log" -> "Timestamp [CARD]in the log").
  { id: 'credit-card', re: /\b\d(?:[ -]?\d){12,18}\b/g, replacement: '[CARD]' },
  ...
  // The negative lookahead exempts 40-char hex, i.e. every git SHA-1. Without
  // it this heuristic redacted the commit hashes that fill this KB's
  // architecture notes and session transcripts — real knowledge destroyed on
  // ingest, unrecoverably. An actual AWS secret drawn entirely from [0-9a-f]
  // has probability (22/64)^40 ~ 1e-19, so nothing is given up.
  { id: 'aws-secret', re: /\b(?![0-9a-fA-F]{40}\b)[A-Za-z0-9/+=]{40}\b/g, replacement: '[AWS_SECRET?]' },
...
export function loadCustomRules(kbRoot) {
  ...
  let cfg
  try {
    cfg = yamlParse(fs.readFileSync(file, 'utf8')) || {}
  } catch (err) {
  ...
  for (const r of cfg.rules || []) {
    if (!r || !r.id || !r.pattern) continue
    // Compile per rule: one invalid pattern or flag string must not silently
    // drop every other custom rule (this loop used to share the file-level
    // try, so a single bad regex disabled all client-name redaction).
    try {
      ...
      console.error(`[redaction] skipping invalid rule "${r.id}": ${err.message}`)
    }
  }
  return out
}
