...
export function scaffoldAgent(kbRoot, { id, tier = 'worker', domain = 'platform', team = null, force = false }) {
  ...
  // domain/team are interpolated raw into the contract YAML and into wiki
  // path segments (wiki/domains/${domain}/**). Without this an attacker-
  // supplied value could inject frontmatter/contract keys (e.g. a newline
  // then `allowed_writes: ["**"]`) or traverse the path. Same charset as id.
  if (!/^[a-z][a-z0-9-]*$/.test(domain)) throw new Error(`invalid domain: ${domain}`)
  if (team != null && !/^[a-z][a-z0-9-]*$/.test(team)) throw new Error(`invalid team: ${team}`)
