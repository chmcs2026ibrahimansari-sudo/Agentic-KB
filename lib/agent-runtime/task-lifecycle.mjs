...
export function workingMemoryPath(contract, taskId) {
  // taskId becomes a filename; a separator or dot-dot here escapes the
  // agent's working-memory directory (appendTaskState/abandonTask write
  // to this path without a contract guard).
  if (typeof taskId !== 'string' || taskId.length === 0 || /[/\\]|\.\./.test(taskId) || taskId.includes('\0')) {
    throw new Error(`Invalid task id: ${String(taskId).slice(0, 80)}`)
  }
 ...
 *   - force?: boolean        (start even if another task is active — orphans it)
 *   - project?: string
 *   - description?: string
 *   - tags?: string[]
...
export function startTask(kbRoot, contract, opts = {}) {
  ...
  // Refuse to silently orphan an in-flight task: overwriting the pointer
  // while another task is active leaves a second status:active working file
  // behind, which verifyTaskState flags as a non-repairable error.
  const existing = getActiveTask(kbRoot, contract)
  if (existing && !opts.force) {
    throw new Error(
      `startTask: agent ${contract.agent_id} already has active task ${existing.taskId}; ` +
      'close or abandon it first (or pass force: true to orphan it deliberately)',
    )
  }

  const explicitId = Boolean(opts.taskId)
  let id = opts.taskId || `task-${timestamp()}`
  let wmRel = workingMemoryPath(contract, id)
  ...
  let wmGuard = assertWriteAllowed(wmRel, contract, guardVars)
  ...
  // Create working-memory file — exclusive create ('wx'): a plain write
  // silently overwrote an existing working-memory file when a caller reused
  // an explicit task_id (destroying a completed task's archived state), and
  // likewise on a same-second cross-process auto-id collision (timestamp()'s
  // uniqueness counter is per-process). Explicit reuse errors; auto ids retry.
  let wmFull = path.join(kbRoot, wmRel)
  fs.mkdirSync(path.dirname(wmFull), { recursive: true })

  const buildWmContent = () => serializeFrontmatter({
  ...
  for (let attempt = 0; ; attempt++) {
    try {
      fs.writeFileSync(wmFull, buildWmContent(), { flag: 'wx' })
      break
    } catch (err) {
      if (err.code !== 'EEXIST') throw err
      if (explicitId || attempt >= 20) {
        throw new Error(
          `startTask: working-memory already exists for task ${id}; ` +
          'refusing to overwrite it (pick a different task_id)',
        )
      }
      id = `task-${timestamp()}`
      wmRel = workingMemoryPath(contract, id)
      wmGuard = assertWriteAllowed(wmRel, contract, guardVars)
      if (!wmGuard.allowed) throw new Error(`startTask: cannot write working-memory: ${wmGuard.reason}`)
      wmFull = path.join(kbRoot, wmRel)
    }
  }

  // Write active-task pointer atomically (tmp + rename, same as every other
  // pointer write in this file): getActiveTask parses this file, so it must
  // never be observable half-written.
  const atFull = path.join(kbRoot, atRel)
  fs.mkdirSync(path.dirname(atFull), { recursive: true })
  const atTmp = atFull + '.tmp-' + process.pid + '-' + Date.now()
  fs.writeFileSync(atTmp, serializeFrontmatter({
    task_id: id,
    project,
    description,
    status: 'active',
...
export function appendTaskState(kbRoot, contract, taskId, entry) {
  ...
  // Atomic write: tmp + rename protects readers from seeing a half-written file.
  const tmp = wmFull + '.tmp-' + process.pid + '-' + Date.now()
...
export function abandonTask(kbRoot, contract, taskId, reason = '') {
  ...
  // Mark working-memory as abandoned. Only an active task can be abandoned:
  // without this check, abandoning a completed/abandoned task silently
  // rewrote its final status (destroying audit history).
  const wm = fs.readFileSync(wmFull, 'utf8')
  const wmStatus = parseFrontmatter(wm).data.status
  if (wmStatus !== 'active') {
    throw new Error(`abandonTask: task ${taskId} is not active (status: ${wmStatus || 'unknown'})`)
  }
  // Atomic tmp + rename (same as appendTaskState): readers must never see a
  // half-written working-memory file.
  const wmTmp = wmFull + '.tmp-' + process.pid + '-' + Date.now()
  fs.writeFileSync(wmTmp, updateFrontmatter(wm, {
    status: 'abandoned',
    abandoned_at: now,
    abandon_reason: reason || null,
    updated: now,
  }))
  fs.renameSync(wmTmp, wmFull)

  // Clear the active-task pointer (keep file, just mark cleared) — but only
  // when it actually points at this task. Clearing unconditionally orphaned
  // the *current* active task when an older task id was abandoned (e.g.
  // after startTask({ force: true })), leaving its working-memory stranded
  // as status:active — exactly what verifyTaskState flags.
  if (fs.existsSync(atFull)) {
    const at = fs.readFileSync(atFull, 'utf8')
    if (parseFrontmatter(at).data.task_id === taskId) {
      // Atomic like the working-memory write above — the pointer is what
      // getActiveTask parses, so it must never be observable half-written.
      const atTmp = atFull + '.tmp-' + process.pid + '-' + Date.now()
      fs.writeFileSync(atTmp, updateFrontmatter(at, {
        status: 'cleared',
        cleared_at: now,
...
export function repairTaskState(kbRoot, contract) {
  ...
  if (activeWorkingFiles.length === 1) {
    ...
    if (!pointerMeta || pointerMeta.status !== 'active' || pointerMeta.working_memory !== activeFile.path) {
      ...
      // Atomic like the startTask/abandonTask pointer writes — the pointer is
      // what getActiveTask parses, so repair must never half-write it.
      const rebuildTmp = pointerFull + '.tmp-' + process.pid + '-' + Date.now()
      fs.writeFileSync(rebuildTmp, serializeFrontmatter({
      ...
      fs.renameSync(rebuildTmp, pointerFull)
      actions.push({ op: 'rebuild-active-task-pointer', path: pointerPath, working_memory: activeFile.path })
    }
  }

  // Re-read the pointer: the block above may have just rebuilt it, and acting
  // on the stale pre-repair metadata here would clear the fresh pointer.
  const currentPointerMeta = readFrontmatterIfExists(pointerFull)
  if (currentPointerMeta?.status === 'active' && currentPointerMeta.working_memory) {
    const wmFull = path.join(kbRoot, currentPointerMeta.working_memory)
    const wmMeta = readFrontmatterIfExists(wmFull)
    if (!wmMeta || wmMeta.status !== 'active') {
      ...
      const clearTmp = pointerFull + '.tmp-' + process.pid + '-' + Date.now()
      fs.writeFileSync(clearTmp, updateFrontmatter(current, {
        status: 'cleared',
        cleared_at: new Date().toISOString(),
        cleared_reason: 'repair-invalid-pointer',
      }))
      fs.renameSync(clearTmp, pointerFull)
