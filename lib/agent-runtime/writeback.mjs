...
import { planAdrOpsForDecisions } from './adr-emitter.mjs'
...
export function _planWrites(kbRoot, contract, vars, payload) {
  ...
  // ── ADR auto-emit on Sofie decisions ──────────────────────────────────────
  // Only Sofie (or any agent with vault_writes) emits ADRs. Workers don't.
  if (Array.isArray(contract.vault_writes) && contract.vault_writes.length > 0) {
    const adrOps = planAdrOpsForDecisions(kbRoot, contract, payload.decisions || [])
    for (const op of adrOps) writes.push(op)
  }
...
export function closeTask(kbRoot, contract, payload) {
  ...
  if (payload && payload.dryRun === true) {
    // `ok` must track the plan — a rejected plan is not a successful call.
    const plan = dryRunCloseTask(kbRoot, contract, payload)
    return { ok: plan.wouldSucceed !== false, dryRun: true, ...plan }
  }
...
export function dryRunCloseTask(kbRoot, contract, payload) {
  ...
  return {
    ...
    summary: {
      ...
      // Count actual allowed plan entries — `rejected` also contains vault
      // fan-out rejections, so `planned.length - rejected.length` undercounts
      // (and can go negative) when a vault op is rejected.
      allowed: planned.filter(d => d.allowed).length,
      rejected: rejected.length,
