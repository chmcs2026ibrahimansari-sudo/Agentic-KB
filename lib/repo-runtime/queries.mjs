...
export function resolveRepoWikiDir(kbRoot, repoName) {
  // repoWikiRoot validates repoName (traversal, separators, null bytes) and
  // must run first so an unsafe name throws rather than reaching readdir.
  ...
  // Match against the directory listing rather than existsSync. On a
  // case-insensitive filesystem (APFS, NTFS) existsSync('…/demo-repo') is true
  // when the directory on disk is 'Demo-Repo', so the old existsSync fast path
  // returned the caller's casing and never reached the canonicalising fallback
  // — defeating the whole point of this function on macOS and Windows.
  const wanted = String(repoName)
  const exact = entries.find(e => e.name === wanted)
  if (exact) return path.join(parent, exact.name)

  const lowered = wanted.toLowerCase()
  const hit = entries.find(e => e.isDirectory() && e.name.toLowerCase() === lowered)
