...
// Atomic tmp + rename (same stance as the agent-runtime writeback and the
// bus/registry writes): a crash mid-write must never leave a truncated
// progress/hot/task file behind.
...
function planRepoWrites(contract, vars, payload, repoName) {
  ...
  for (const w of writes) {
    ...
    // Fall back to the repo-scoped rules only when the global guard's answer
    // was "this path isn't in your allowlist" — including the empty-allowlist
    // case, which previously skipped the fallback entirely and blocked
    // repo-only agents (no global allowed_writes) from their own progress/
    // memory/bus paths. Unsafe-path and forbidden_paths rejections stay final.
    const fellThrough = !globalDecision.allowed &&
      (globalDecision.reason === 'not in allowed_writes' || globalDecision.reason === 'no allowed_writes configured')
    const d = fellThrough
...
function commitRepoWrite(kbRoot, repoName, contract, write, trace) {
  ...
  if (write.op === 'append' || (write.op !== 'replace' && isAppendOnly(cls))) {
    ...
    atomicWrite(full, write.content)
  }
...
export function closeRepoTask(kbRoot, repoName, contract, payload) {
  ...
  // Commit under an exclusive per-repo lock — the agent-runtime closeTask has
  // done this since Phase 3, but the repo twin committed bare, so two agents
  // closing tasks on the same repo could interleave the read-modify-write
  // appends to progress.md and silently drop one side's entry.
  const rollbacks = []
  let commitError = null
  try {
    withLock(kbRoot, `repo:${repoName}`, () => {
...
export function appendRepoProgress(kbRoot, repoName, entry, agentId) {
  ...
  // Same exclusive per-repo lock closeRepoTask takes for its identical
  // read-modify-write of this file. Without it, this exported twin racing
  // closeRepoTask (or another appendRepoProgress) reads the pre-append
  // content and the atomic rename then replaces the other side's committed
  // entry wholesale — a total loss, not a partial one.
  return withLock(kbRoot, `repo:${repoName}`, () => {
    const existing = readOrInit(progressPath, { memory_class: 'learned', repo: repoName })
    const sep = `\n\n## ${new Date().toISOString()}\n`
    atomicWrite(progressPath, existing.endsWith('\n') ? existing + sep + entry + '\n' : existing + '\n' + sep + entry + '\n')
    return path.relative(kbRoot, progressPath)
  })
}

export function writeRepoTaskLog(kbRoot, repoName, taskId, agentId, content) {
  // taskId becomes a filename under tasks/YYYY/MM/DD/ — reject separators
  // and dot-dot so a crafted id cannot escape the repo namespace.
  ...
  atomicWrite(taskPath, serializeFrontmatter(fm, '\n' + content + '\n'))
